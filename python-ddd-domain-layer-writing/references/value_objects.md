...
- **Предикаты `is_X()` для каждого значения** — обязательно. Использование:
  ```python
...
**Имя предиката:** `is_<value>()`, всегда без отрицания. Не `is_not_deleted` — пишите `not state.is_deleted()`.
...
| Где живёт | Когда |
|---|---|
| `domain/value_objects.py` | используется ≥ 2 разными агрегатами/проекциями ИЛИ базовыми классами |
| `domain/<aggregate>/value_objects.py` | используется только этим агрегатом |
