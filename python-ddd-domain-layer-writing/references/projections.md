...
| Откат версии назад | Невозможен | Защищён `EntityVersionLessThenCurrentError` |
...
| `version < self._version` | `EntityVersionLessThenCurrentError` — пришло устаревшее событие, отбрасываем |
...
- Если меньше — `EntityVersionLessThenCurrentError` (устаревшее событие).
- Если больше — применит.

Use case должен решать, как обработать каждый случай: `EntityIdempotentError` обычно — log-and-skip, `EntityVersionLessThenCurrentError` — log-and-skip с алертом, успех — продолжить.
...
- Создаём свой enum в `domain/<projection>/value_objects.py` (`UserState`).
...
# domain/tenant/value_objects.py
@dataclass(frozen=True)
class TenantID:
    tenant_id: UUID

# domain/tenant/services.py
async def create(self, user: User) -> Tenant:
    existing = await self._read_repo.by_id(TenantID(user.user_id.user_id))   # ID parity
    if existing:
...
**Имя метода:** `restore` — потому что и из БД, и из события мы восстанавливаем известное состояние, не создаём новое локальное. Если хочется акцентировать роль приёма событий, допустим `from_data` — но `restore` единообразен с фабриками агрегатов.

**Вызывают:**
- Репозиторий проекции — при чтении из БД.
- Обработчик события (use case в application-слое) — при создании проекции по событию «X создан». В обоих случаях все данные приходят с одного входа, фабрике это безразлично.
