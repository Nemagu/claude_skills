...
- Response-DTO (`TenantSimpleDTO` и т.п. — они описывают **выход** use case, не вход).
...
| ✓ Разрешено | ✗ Запрещено |
|---|---|
| `-> TenantSimpleDTO` | `-> Tenant` (domain entity) |
| `-> tuple[list[TenantSimpleDTO], int]` | `-> list[Tenant]` |
...
| Расположение | `commands/{public,private}/<aggregate>/<action>.py` | `queries/public/<aggregate>/<action>.py` |
| Suffix dataclass | `Command` | `Query` |
| Verb form | императив (`Create`, `Update`, ...) | `Get` / `List` |
| Транзакционная граница | один `async with` на `execute` | допустимо несколько последовательных |
...
class ListTenantLastVersionsUseCase(BaseUseCase):
    async def execute(
        ...
        async with self._uow as uow:
            ...
            return [
                TenantSimpleDTO.from_domain(tenant) for tenant in tenants
            ], count
