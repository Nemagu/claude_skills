...
| `application.dto.paginators`, `application.errors` | `presentation.*` |
...
└── repositories/
    ├── __init__.py                  ← НЕ пустой: <Aggregate>Repositories группы
    └── <aggregate>.py               ← per-aggregate набор интерфейсов
```
...
- `publish` — единичная публикация, `batch_publish` — массовая. Оба обязательны.
- Используется только publisher use case-ами.

## `repositories/<aggregate>.py` — каркас файла
...
## Группы `<Aggregate>Repositories` (`ports/repositories/__init__.py`)
...
**Правила:**
- Файл `ports/repositories/__init__.py` — **единственный непустой** `__init__.py` в слое.
- В нём же `__all__` со всеми именами всех `ports/repositories/<aggregate>.py`.
